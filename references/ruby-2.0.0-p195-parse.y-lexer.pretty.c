...
static NODE *logop_gen(struct parser_params *, enum node_type, NODE *,
                       NODE *);
...
static NODE *new_args_gen(struct parser_params *, NODE *, NODE *, ID, NODE *,
                          NODE *);
...
static NODE *new_op_assign_gen(struct parser_params *parser, NODE * lhs,
                               ID op, NODE * rhs);
static NODE *new_attr_op_assign_gen(struct parser_params *parser, NODE * lhs,
                                    ID attr, ID op, NODE * rhs);
#define new_attr_op_assign(lhs, type, attr, op, rhs) new_attr_op_assign_gen(parser, (lhs), (attr), (op), (rhs))
static NODE *new_const_op_assign_gen(struct parser_params *parser, NODE * lhs,
                                     ID op, NODE * rhs);
...
static VALUE new_op_assign_gen(struct parser_params *parser, VALUE lhs,
                               VALUE op, VALUE rhs);
static VALUE new_attr_op_assign_gen(struct parser_params *parser, VALUE lhs,
                                    VALUE type, VALUE attr, VALUE op,
                                    VALUE rhs);
...
static VALUE ripper_dispatch3(struct parser_params *, ID, VALUE, VALUE,
                              VALUE);
static VALUE ripper_dispatch4(struct parser_params *, ID, VALUE, VALUE, VALUE,
                              VALUE);
static VALUE ripper_dispatch5(struct parser_params *, ID, VALUE, VALUE, VALUE,
                              VALUE, VALUE);
static VALUE ripper_dispatch7(struct parser_params *, ID, VALUE, VALUE, VALUE,
...
static inline VALUE
new_args_gen(struct parser_params *parser, VALUE f, VALUE o, VALUE r, VALUE p,
             VALUE tail)
{
  NODE *t = (NODE *) tail;
...
static void ripper_warningS(struct parser_params *, const char *,
                            const char *);
#endif
...
static int parser_tokadd_string(struct parser_params *, int, int, int, long *,
                                rb_encoding **);
static void parser_tokaddmbc(struct parser_params *parser, int c,
                             rb_encoding * enc);
...
parser_yyerror(struct parser_params *parser, const char *msg)
{
  ...
  if (len > 4)
  {
    ...
    if (len > max_line_margin * 2 + 10)
    {
      ...
      {
        p =
          rb_enc_prev_char(p, lex_p - max_line_margin, pe,
      ...
      {
        pe =
          rb_enc_prev_char(lex_p, lex_p + max_line_margin, pe,
                           rb_enc_get(lex_lastline));
        post = "...";
      }
      len = pe - p;
...
debug_lines(const char *f)
{
  ...
  if (rb_const_defined_at(rb_cObject, script_lines))
  {
    ...
    {
      VALUE fname =
        rb_external_str_new_with_enc(f, strlen(f), rb_filesystem_encoding());
...
yycompile0(VALUE arg)
{
  ...
  deferred_nodes = 0;
#ifndef RIPPER
  parser->parser_token_info_enabled = !compile_for_eval
    && RTEST(ruby_verbose);
#endif
#ifndef RIPPER
...
NODE *
rb_parser_compile_cstr(volatile VALUE vparser, const char *f, const char *s,
                       int len, int line)
{
  VALUE str = rb_str_new(s, len);
...
parser_tokadd_mbchar(struct parser_params *parser, int c)
{
  ...
  {
    compile_error(PARSER_ARG "invalid multibyte char (%s)",
      ...
      switch (c)
      {
        ...
        case 'u':
          ...
          parser_tokadd_utf8(parser, &enc, 1,
                             func & STR_FUNC_SYMBOL, func & STR_FUNC_REGEXP);
...
RUBY_FUNC_EXPORTED const unsigned int
...
parser_set_encode(struct parser_params *parser, const char *name)
{
  ...
  error:
    ...
    rb_ary_unshift(excargs[2],
                   rb_sprintf("%s:%d", ruby_sourcefile, ruby_sourceline));
...
#ifndef RIPPER
typedef long (*rb_magic_comment_length_t) (struct parser_params * parser,
                                           const char *name, long len);
typedef void (*rb_magic_comment_setter_t) (struct parser_params * parser,
                                           const char *name, const char *val);

static void
magic_comment_encoding(struct parser_params *parser, const char *name,
                       const char *val)
{
...
static void
parser_set_token_info(struct parser_params *parser, const char *name,
                      const char *val)
{
...
static void
set_file_encoding(struct parser_params *parser, const char *str,
                  const char *send)
{
  int sep = 0;
  const char *beg = str;
...
#define no_digits() do {yyerror("numeric literal without digits"); return 0;} while (0)
      ...
      switch (c)
      {
        ...
        default:
          if (!parser_is_identchar())
          {
            pushback(c);
            compile_error(PARSER_ARG
                          "`$%c' is not allowed as a global variable name",
                          c);
            return 0;
          }
        case '0':
    ...
    default:
      if (!parser_is_identchar())
      {
        rb_compile_error(PARSER_ARG "Invalid char `\\x%02X' in expression",
                         c);
        goto retry;
      }
...
#ifndef RIPPER
static NODE *
node_newnode(struct parser_params *parser, enum node_type type, VALUE a0,
             VALUE a1, VALUE a2)
{
...
gettable_gen(struct parser_params *parser, ID id)
{
  switch (id)
  {
    ...
    case keyword__FILE__:
      return
        NEW_STR(rb_external_str_new_with_enc
                (ruby_sourcefile, strlen(ruby_sourcefile),
...
new_bv_gen(struct parser_params *parser, ID name)
{
  ...
  if (!is_local_id(name))
  {
    compile_error(PARSER_ARG "invalid local variable - %s", rb_id2name(name));
    return;
  }
  shadowing_lvar(name);
  dyna_var(name);
...
arg_concat_gen(struct parser_params *parser, NODE * node1, NODE * node2)
{
  ...
  switch (nd_type(node1))
  {
    ...
    case NODE_ARGSCAT:
      if (nd_type(node2) != NODE_ARRAY ||
          nd_type(node1->nd_body) != NODE_ARRAY)
        break;
...
is_static_content(NODE * node)
{
  ...
  {
    case NODE_HASH:
      if (!(node = node->nd_head))
        break;
    case NODE_ARRAY:
      do
      {
        if (!is_static_content(node->nd_head))
          return 0;
      }
...
static void
warn_unless_e_option(struct parser_params *parser, NODE * node,
                     const char *str)
{
  if (!e_option_supplied(parser))
...
static void
warning_unless_e_option(struct parser_params *parser, NODE * node,
                        const char *str)
{
  if (!e_option_supplied(parser))
    parser_warning(node, str);
}
...
static NODE *
logop_gen(struct parser_params *parser, enum node_type type, NODE * left,
          NODE * right)
{
  value_expr(left);
  if (left && (enum node_type) nd_type(left) == type)
  {
    ...
    while ((second = node->nd_2nd) != 0
           && (enum node_type) nd_type(second) == type)
    {
      node = second;
    }
...
static NODE *
new_args_gen(struct parser_params *parser, NODE * m, NODE * o, ID r, NODE * p,
             NODE * tail)
{
  int saved_line = ruby_sourceline;
...
static NODE *
new_attr_op_assign_gen(struct parser_params *parser, NODE * lhs, ID attr,
                       ID op, NODE * rhs)
{
  NODE *asgn;
...
static NODE *
new_const_op_assign_gen(struct parser_params *parser, NODE * lhs, ID op,
                        NODE * rhs)
{
  NODE *asgn;
...
#else
static VALUE
new_op_assign_gen(struct parser_params *parser, VALUE lhs, VALUE op,
                  VALUE rhs)
{
...
static VALUE
new_attr_op_assign_gen(struct parser_params *parser, VALUE lhs, VALUE type,
                       VALUE attr, VALUE op, VALUE rhs)
{
...
warn_unused_var(struct parser_params *parser, struct local_vars *local)
{
  ...
  for (i = 0; i < cnt; ++i)
  {
    if (!v[i] || (u[i] & LVAR_USED))
      continue;
    if (is_private_local_id(v[i]))
      continue;
    rb_warn4S(ruby_sourcefile, (int) u[i],
              "assigned but unused variable - %s", rb_id2name(v[i]));
  }
}
...
local_push_gen(struct parser_params *parser, int inherit_dvars)
{
  ...
  local->used = !(inherit_dvars &&
                  (ifndef_ripper
                   (compile_for_eval || e_option_supplied(parser)) + 0))
...
static int
reg_named_capture_assign_iter(const OnigUChar * name,
                              const OnigUChar * name_end, int back_num,
                              int *back_refs, OnigRegex regex, void *arg0)
{
                                                          ...
                                                          NEW_CALL(gettable
                                                                   ...
                                                                   (NEW_LIT
                                                                    (ID2SYM
                                                                     (var)))))));
  arg->fail_block =
    block_append(arg->fail_block,
                 newline_node(node_assign
                              (assignable(var, 0), NEW_LIT(Qnil))));
  return ST_CONTINUE;
}
                             ...
                             NODE * match)
{
  ...
  arg.fail_block = 0;
  arg.num = 0;
  onig_foreach_name(RREGEXP(regexp)->ptr, reg_named_capture_assign_iter,
                    (void *) &arg);
  ...
  return
    block_append(newline_node(match),
                 NEW_IF(gettable(rb_intern("$~")),
                        block_append(newline_node(arg.succ_block),
                                     newline_node(NEW_CALL
                                                  (gettable(rb_intern("$~")),
                                                   rb_intern("begin"),
                                                   NEW_LIST(NEW_LIT
                                                            (INT2FIX(0)))))),
                        block_append(newline_node(arg.fail_block),
                                     newline_node(NEW_LIT(Qnil)))));
}
...
reg_compile_gen(struct parser_params *parser, VALUE str, int options)
{
  ...
  err = rb_errinfo();
  re =
    rb_reg_compile(str, options & RE_OPTION_MASK, ruby_sourcefile,
                   ruby_sourceline);
  if (NIL_P(re))
  {
...
rb_gc_mark_symbols(void)
{
  ...
  rb_gc_mark_locations(global_symbols.op_sym,
                       ...
                       numberof(global_symbols.op_sym));
}
#endif /* !RIPPER */
  ...
  switch (*m)
  {
    ...
    id:
      ...
      switch (*m)
      {
        case '!':
        case '?':
          if (type == ID_GLOBAL || type == ID_CLASS || type == ID_INSTANCE)
            return -1;
          type = ID_JUNK;
          ++m;
          break;
        case '=':
          if (type != ID_CONST && type != ID_LOCAL)
            return -1;
          type = ID_ATTRSET;
          ++m;
          break;
...
new_id:
  if (symenc != enc)
    rb_enc_associate(str, symenc);
  if (global_symbols.last_id >=
      ~(ID) 0 >> (ID_SCOPE_SHIFT + RUBY_SPECIAL_SHIFT))
  {
    if (len > 20)
    {
...
rb_check_id(volatile VALUE * namep)
{
  ...
  else if (!RB_TYPE_P(name, T_STRING))
  {
    ...
    {
      tmp = rb_inspect(name);
      rb_raise(rb_eTypeError, "%s is not a symbol", RSTRING_PTR(tmp));
    }
    name = tmp;
    *namep = name;
  }
...
parser_memsize(const void *ptr)
{
  ...
  for (local = p->parser_lvtbl; local; local = local->prev)
  {
    size += sizeof(*local);
    if (local->vars)
      size += local->vars->capa * sizeof(ID);
  }
#ifndef RIPPER
...
static VALUE
ripper_dispatch3(struct parser_params *parser, ID mid, VALUE a, VALUE b,
                 VALUE c)
{
  validate(a);
  validate(b);
...
static VALUE
ripper_dispatch4(struct parser_params *parser, ID mid, VALUE a, VALUE b,
                 VALUE c, VALUE d)
{
  validate(a);
  validate(b);
...
static VALUE
ripper_dispatch5(struct parser_params *parser, ID mid, VALUE a, VALUE b,
...
{
  NODE *nd;
  if (!RB_TYPE_P(v, T_NODE))
    return 0;
  nd = (NODE *) v;
  if (nd_type(nd) != NODE_LASGN)
    return 0;
  return nd->nd_vid;
}
...
{
  rb_funcall(parser->value, rb_intern("warn"), 2, STR_NEW2(fmt), INT2NUM(a));
}
...
static void
ripper_warningS(struct parser_params *parser, const char *fmt,
                const char *str)
{
...
ripper_parse0(VALUE parser_v)
{
  ...
  TypedData_Get_Struct(parser_v, struct parser_params, &parser_data_type,
                       parser);
  parser_prepare(parser);
